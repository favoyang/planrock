...
(async () => {
  try {
    ...
    const health = await get("/api/health"); assert.equal(health.status, 200); assert.equal(JSON.parse(health.body).service, "planrock"); assert.equal(health.headers["access-control-allow-origin"], "*");
