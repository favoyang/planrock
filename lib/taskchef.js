...
function importTaskChef(registry) {
  const diagnostics = [];
  ...
  try {
    ...
    diagnostics.push(diagnostic("TASKCHEF_IMPORT_INVALID", "warning", `TaskChef discovery skipped: ${error.message}`));
  ...
  for (const record of parsed.records) {
    try {
      ...
      if (preparedRoots.has(root) || registry.suppressions.taskchef.includes(root)) continue;
