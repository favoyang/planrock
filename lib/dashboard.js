...
function planSourceGuards(snapshot, plan) {
  ...
  return [repository.root, path.join(repository.root, "plans")].map((target) => {
    const before = fs.lstatSync(target);
    if (before.isSymbolicLink() || !before.isDirectory() || fs.realpathSync.native(target) !== target) throw new Error("Indexed plan directory is no longer valid");
    const after = fs.lstatSync(target);
    if (after.isSymbolicLink() || after.dev !== before.dev || after.ino !== before.ino) throw new Error("Indexed plan directory changed during inspection");
