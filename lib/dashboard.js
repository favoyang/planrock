...
const { fingerprint, loadIndex, pageCollection } = require("./indexer");
...
async function serveDashboard(port) {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (url.pathname.startsWith("/api/")) {
      if (url.pathname === "/api/overview" && req.method === "GET") { const nextUp = pageCollection(snapshot, "openPlans", { limit: 200 }); json(res, 200, { schemaVersion: 1, snapshotId: snapshot.snapshotId, refreshedAt: snapshot.refreshedAt, incomplete: snapshot.incomplete, nativeActions: nativeActionsAvailable(req), health: healthState, summary: snapshot.summary, diagnostics: snapshot.diagnostics.slice(0, 100), nextUp: nextUp.items, omittedNextUp: snapshot.openPlans.length - nextUp.items.length }, securityHeaders()); return; }
      ...
      if (url.pathname === "/api/refresh" && req.method === "POST") { if (!localClient(req)) { json(res, 403, { error: "LOCAL_REFRESH_REQUIRED" }, securityHeaders()); return; } await refresh(); json(res, 200, { snapshotId: snapshot.snapshotId, refreshedAt: snapshot.refreshedAt, health: healthState }, securityHeaders()); return; }
