...
function nativeActionAllowed(req) {
  ...
  if (!origin || !isLoopbackHost(req.socket.remoteAddress?.replace(/^::ffff:/, ""))) return false;
...
function markdownSource(snapshot, plan, requestedPath) {
  const repository = indexedPlanRepository(snapshot, plan);
